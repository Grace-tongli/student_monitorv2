...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(BASE_DIR, "logs/monitor_system.log")),
        logging.StreamHandler()
    ]
)

def run_monitors(username):
    """运行整个监控系统"""
    logging.info(f"编程行为监控系统启动，用户: {username}")
    ...
    try:
        ...
        kb_monitor = KeyboardMonitor(
            ...
            output_file=os.path.join(BASE_DIR, "data", f"{username}_keyboard_performance.csv"),
        ...
        mouse_monitor = MouseMonitor(
            analysis_interval=120,
            output_file=os.path.join(BASE_DIR, "data", f"{username}_mouse_performance.csv"),
            stop_event=stop_event
        )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("用法: python run_monitors.py <用户名>")
        sys.exit(1)

    username = sys.argv[1]
    run_monitors(username)